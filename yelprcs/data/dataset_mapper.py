...
class YelpMeanTableMapper(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        user_id, business_id, star = dataset_dict
        dataset_dict.extend([self.mean_tables['whole_mean'], self.mean_tables['user_mean'][user_id], self.mean_tables['business_mean'][business_id]])
        return dataset_dict
